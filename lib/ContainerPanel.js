...
GeoViewer.ContainerPanel = Ext.extend(
    Ext.Panel,
    {
        ...
        createWMSCapabilitiesPanel : function(options){
            var myWMSCapabilitiesBrowser = new OpenGeoGroep.framework.WMSCapabilitiesBrowser;
            if(ogg && (ogg.services instanceof Array))
            {
                for(var i = 0; i <  ogg.services.length; i++)
                {
                    var service = ogg.services[i];
                    myWMSCapabilitiesBrowser.addService(service);
                }
                return myWMSCapabilitiesBrowser;
            }
            return null;
        },
                ...
                buttons: [
                {
                    ...
                    handler: function(){
                        ...
                        Ext.Ajax.request({
                            ...
                            method: 'POST',
                            params:{
                                'cql_filter':paramString,
                                'typename': 'maasdonk:adressen'
                            },
    ...
    createPanel : function(type, options) {
        switch (type) {
            ...
            case 'gv-html':
                // Standard HTML Panel
                return this.createHTMLPanel(options);
            case 'gv-wmscapabilities-browser':
                //pass the services that need to be handled as options
                return this.createWMSCapabilitiesPanel(options);
